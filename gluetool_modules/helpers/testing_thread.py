...
class TestingThread(gluetool.Module):
    ...
    a SHA1 hash of string, consisting of time and optionally variables from `eval_context`, which
    are choose by `construct_from` option eg. Jenkins-related properties (master hostname, job name,
    etc.) or artifact related properties (build ID).
    ...
    required_options = ['construct-from']
    ...
    options = {
        ...
        'construct-from': {
            'help': 'A comma separated list of variable names from `eval_context`, which are used to construct id',
            'type': str
        },
        'id-length': {
    ...
    def execute(self):
        ...
        fmt = gluetool.utils.normalize_multistring_option(self.option('construct-from'))

        eval_context = self.shared('eval_context')
        variables = gluetool.utils.dict_update(eval_context,
                                               eval_context['ENV'],
                                               {'STAMP': int(time.time())})
