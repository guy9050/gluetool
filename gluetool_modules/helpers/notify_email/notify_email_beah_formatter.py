...
class NotifyEmailBeahFormatter(gluetool.Module):
    """
        ...
        for name, runs in result.payload.iteritems():
            ...
            for run in runs:
                ...
                if name not in fails:
                    ...
                    if name_parts[1] == 'distribution':
                        test_src = 'http://pkgs.devel.redhat.com/cgit/tests/distribution/tree/{}'.format(
                            '/'.join(name_parts[2:]))
                        test_src = gluetool.utils.treat_url(test_src, logger=self.logger)
                ...
                if first_full_log is not None:
                    run_summary['testout.log'] = gluetool.utils.treat_url(first_full_log['href'], logger=self.logger)
                ...
                for i, phase in enumerate(run['bkr_phases']):
                    ...
                    run_summary['phases'].append({
                        ...
                        'logs': [gluetool.utils.treat_url(log['href'], logger=self.logger) for log in phase['logs']]
                    })

                fails[name].append(run_summary)

        self.debug('found fails:\n{}'.format(gluetool.log.format_dict(fails)))
    ...
    def notify_email_beaker_formatter(self, notify_email, result, message):
        ...
        :param gluetool_modules.testing.beaker.beaker.BeakerTestResult result: Beaker result.
    ...
    def notify_email_restraint_formatter(self, notify_email, result, message):
        ...
        :param gluetool_modules.helpers.notify_email.Notify: ``notify-email`` module governing this operation.
        :param gluetool_modules.testing.restraint.runner.RestraintTestResult result: Restraint result.
        :param gluetool_modules.helpers.notify_email.Message: E-mail message container.
        """
