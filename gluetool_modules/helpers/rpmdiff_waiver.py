...
class RpmDiffWaiver(gluetool.Module):
    """
    ...
    def waive_test(self, link, comment):
        ...
        :raise gluetool.GlueError: if POST request or authentication failed
        """
        # try kerberos login
        ...
        if session.get(url, auth=kerberos_auth).status_code != 200:
            raise GlueError("Authentication failed while waiving RPMDiff tests")
        # obtain token
        ...
        if waive_request.status_code != 200:
            raise GlueError("Test was probably not waived due to error in http, http code is '{}'"
    ...
    def waive_results(self, run_id, package, target):
        ...
        :raises gluetool.GlueError: if *postgresql* module is not included before this module
        :raises gluetool.GlueError: if WebUI does not return expected webpage
        """
        ...
        if not table:
            raise GlueError('table of results was not found on RPMDiff WebUI')
