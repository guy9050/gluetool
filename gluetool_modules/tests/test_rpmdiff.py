...
def fixture_module():
    return create_module(gluetool_modules.static_analysis.rpmdiff.rpmdiff.CIRpmdiff)
...
def test_run_command(module, monkeypatch):
    _, module = module
    run_mock = MagicMock()
    monkeypatch.setattr(gluetool.utils, "run_command", run_mock)
...
def test_run_command_fail(module, monkeypatch):
    ...
    with pytest.raises(gluetool.GlueError, match=r"^Failure during 'rpmdiff-remote' execution:"):
        module._run_command(["arg0", "arg1", "arg2"])
...
def test_wait_until_finished_timeout_exceed(module, monkeypatch):
    ...
    monkeypatch.setattr(gluetool.utils, "run_command", run_mock)
    with pytest.raises(gluetool.GlueError, match=r"^Waiting for RPMdiff results timed out"):
...
def test_publish_results(module_with_ciresults, monkeypatch, rpmdiff_test_type, expected_results):
    ...
    monkeypatch.setattr(gluetool.utils, "run_command", passed_run_command)
...
def test_refresh_results_fail(module):
    _, module = module
    with pytest.raises(gluetool.GlueError, match=r"^Shared function 'results' is required."):
        module.refresh_rpmdiff_results(118618)


def test_refresh_results_another_id(module_with_ciresults, monkeypatch):
    ci, module = module_with_ciresults
    monkeypatch.setattr(gluetool.utils, "run_command", failed_run_command)
    assert not ci.shared("results")
