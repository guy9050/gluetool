...
class CICovscan(gluetool.Module):
    ...
    def covscan_xunit_serialize(self, test_suite, result):
        ...
        new_xml_element('log', _parent=logs, name='src.rpm', href=logs_url.format(result.baseline.nvr + '.src.rpm'))
        new_xml_element('log', _parent=logs, name='tar.gz', href=logs_url.format(result.baseline.nvr + '.tar.gz'))
    ...
    def scan(self):
        ...
        if not covscan_result:
            ...
            log_dict(self.info, 'Obtaining source RPMs', {
                'target': target_srpm_url,
                'baseline': "{} -> {}".format(baseline_srpm_url, baseline_srpm_name)
            })
            target_srpm = urlgrab(target_srpm_url)
            baseline_srpm = urlgrab(baseline_srpm_url, filename=baseline_srpm_name)
