...
class ModuleInfoGroup(object):
    ...
    def pip_version_unify(self, pip_deps):
        ...
        for item in pip_deps:
            ...
            if pkg in versions:
                ...
                version_limit = libci.utils.Bunch(pkg=pkg, equal=None, lower=None, upper=None)
        ...
        if item.equal:
            if item.upper:
                ...
                if not relate(Version(item.equal), Version(version)):
                    raise libci.CIError("Cannot find common version for package '{}'".format(item.pkg))
            if item.lower:
    ...
    def limit_version(item, oper, version):
        if oper in ['>', '>=']:
            ...
            raise libci.CIError("Unsupported operator '{}'".format(oper))
...
class DepList(libci.Module):
    ...
    def load_moduleinfo_files(self, files):
        ...
        for filename in files:
            yaml = libci.utils.load_yaml(filename)
            for definition in yaml:
                try:
                    item = ModuleInfo(definition)
                    if loaded.add_moduleinfo(item):
                        self.info("Module info '{}' loaded".format(item.name))
                        self.debug("Module: '{}'\nDescription: '{}'\nDependencies:\n'{}'".format(
                            item.name, item.description, libci.utils.format_dict(item.dependencies)
                        ))
                except KeyError:
    ...
    def prepare_dependencies(self, module_dirs, only_modules):
        ...
        self.debug('Resulting dependencies:\n{}'.format(libci.utils.format_dict(dependencies)))
    ...
    def execute(self):
        ...
        if dirs and outfile:
            ...
            libci.utils.dump_yaml(dependencies['ansible_tasks'], tasks_file)
