...
All templates are given following variables:
    ...
    * ``PRIMARY_TASK`` - instance of :py:class:`libci.modules.infrastructure.koji_fedora.KojiTask`
...
Some templates are given special extra variables:
    ...
    * ``subject``, ``soft-error-message``, ``hard-error-message``
        * ``FAILURE`` - an instance of :py:class:`libci.ci.Failure` describing the issues that caused pipeline
          to fail.
...
class Notify(Module):
    ...
    def email_map(self):
        return libci.utils.PatternMap(self.option('email-map'), logger=self.logger)
    ...
    def render_template(self, filename, **variables):
        ...
        :py:meth:`libci.utils.render_template` to do the rendering job.
        ...
        return render_template(
            ...
            **libci.utils.dict_update(variables, {
                'OS': os,
                'MODULE': self,
    ...
    def format_result_url(self, result, key, default):
        ...
        with '..' and other stuff. Most of the work is offloaded to :py:func:`libci.utils.treat_url`
    ...
    def _format_failure(self, failure):
        ...
        body_header = self.render_template(self.option('body-header-template'), **{
            'SUMMARY_URL': self._get_summary_url(libci.results.TestResult(self.ci, 'dummy', 'ERROR'))
        })
