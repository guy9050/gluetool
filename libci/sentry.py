...
class Sentry(object):
    ...
    def event_url(self, event_id, logger=None):
        ...
        return libci.utils.treat_url('{}/?query={}'.format(self._base_url, event_id), logger=logger)

    def log_issue(self, event_id, logger=None):
        """
        ...
        if not logger or not event_id:
            return

        logger.error("Submitted as Sentry issue '{}'".format(event_id))

        event_url = self.event_url(event_id, logger=logger)

        if event_url:
            logger.error('See {} for details.'.format(event_url))
    ...
    def _capture(self, event_type, logger=None, failure=None, **kwargs):
        ...
        event_id = self._client.capture(event_type, tags=tags, **kwargs)

        if failure is not None:
            failure.sentry_event_id = event_id

        self.log_issue(event_id, logger=logger)
    ...
    def submit_exception(self, failure, logger=None, **kwargs):
        ...
        :param libci.ci.Failure failure: ``Failure`` instance describing the exception.
