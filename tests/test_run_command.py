...
@pytest.mark.parametrize('actual_errno, expected_exc, expected_message', [
    (errno.ENOENT, libci.CIError, "Command '/bin/foo' not found"),
...
def test_exit_code_error(popen, log):
    ...
    with pytest.raises(libci.CICommandError, match=r"^Command '\['/bin/foo'\]' failed with exit code 1$") as excinfo:
        run_command(['/bin/foo'])
